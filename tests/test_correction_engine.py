...
def test_high_confidence_note_is_changed_and_note_off_matches(tmp_path):
    src = tmp_path / 'src.mid'; out = tmp_path / 'out.mid'; _make_midi(src)
    result = apply_note_corrections(src, out, [NoteCorrection(0,0,60,62,.99,'audio match')])
...
def test_low_confidence_note_is_not_changed(tmp_path):
    src = tmp_path / 'src.mid'; out = tmp_path / 'out.mid'; _make_midi(src)
    result = apply_note_corrections(src, out, [NoteCorrection(0,0,60,62,.50,'weak evidence')])
    assert result.applied == 0 and result.skipped == 1
    mid = mido.MidiFile(out)
    assert [m.note for m in mid.tracks[0] if m.type == 'note_on'][0] == 60


def test_source_overwrite_is_refused(tmp_path):
    src = tmp_path / 'src.mid'; _make_midi(src)
